...
watchEffect(() => {
    if (product.value && product.value.data) {
      // get the product data URL assign it to currentImage
...
const isInCart = computed(() => {
    let res = false; // local variable set to false, 
    // get our cart from userStore and loop through it in a forEach
    userStore.cart.forEach(prod => {
        if (route.params.id == prod.id) {
            res = true; // in that case, we added into the cart
        }
    })
    return res
})
...
const priceComputed = computed(() => {
    ...
    return '0.00'; // if is nothing, return nothing
})
