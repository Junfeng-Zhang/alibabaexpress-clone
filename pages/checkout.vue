...
onBeforeMount(async () => {
    if (userStore.checkout.length < 1) {
        return navigateTo('/shoppingcart'); // no need to come to the check out
    }

    total.value = 0.00
    if (user.value) { // if user is logged in
...
const stripeInit = async () => {
    ...
    stripe = Stripe(runtimeConfig.stripePk); // nuxt.config.ts 里面进行配置
